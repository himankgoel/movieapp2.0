...
//@Access Public
 app.post("/api/user/register" , (req,res) => {
    ...
    movieUser.findOne({email})
    .then(user => {
        if(user)
        return res.status(400).json({msg: "Entered email id already exists. Please Login instead."});
       })
       .catch(err => {
           console.log(err);
       });
...
app.post("/api/user/login" , (req,res) => {
   ...
   .then(user => {
       if(!user)
       return res.status(404).json({msg: "Email doesn't exists. Register instead?"});
       //Validate passwords
    ...
    app.post("/api/user/addmovie", (req, res) => {
        try {var decoded = jwt.verify(req.body.token , process.env.JWTSECRET);
            if(req.body.imdbID === null) return;
        movieUser.findByIdAndUpdate(decoded.id, {$push: {favouriteMovies: req.body.imdbID}},{useFindAndModify:false},(err,res) => {
            if(!err){
     ...
     //desc DELETE THE PARTICULAR NOTE FROM DATABASE BY NOTEID 
      app.post("/api/user/deletemovie", (req, res) => {
        try {var decoded = jwt.verify(req.body.token , process.env.JWTSECRET);
            if(req.body.imdbID === null) return;
        movieUser.findByIdAndUpdate(decoded.id, {$pull: {favouriteMovies:req.body.imdbID }},{useFindAndModify:false},(err,res) => {
            if(!err){
